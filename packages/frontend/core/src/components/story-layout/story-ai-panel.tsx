...
const actions = [
  { id: 'continue', label: '续写', prompt: '请根据以下内容续写故事，保持风格和语气一致：\n\n' },
  { id: 'polish', label: '润色', prompt: '请润色以下段落，改善文字表达，保持原意：\n\n' },
...
export const StoryAIPanel = forwardRef<StoryAIHandle, StoryAIPanelProps>(
  ...
  const getActiveChapterContent = useCallback((): string => {
    ...
    const chapter = chapters.find(
      ch => ch.meta.index === activeChapterIndex
    );
  ...
  const hasChapterContent =
    ...
    chapters.some(ch => ch.meta.index === activeChapterIndex && ch.content.trim().length > 0);
