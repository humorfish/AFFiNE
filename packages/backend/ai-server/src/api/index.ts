...
import {
  worldviewPrompts,
  worldviewSectionPrompt,
  parseWorldview,
  parseWorldviewSection,
} from './worldview';
...
import {
  specialSettingsPrompts,
  parseSpecialSetting,
} from './special-settings';
import { charactersPrompts, parseCharacter } from './characters';
import {
  characterRelationsPrompts,
...
export {
  ...
  geoMapsPrompts,
  parseGeoMap,
  foreshadowingPrompts,
  parseForeshadowing,
  plotStructuresPrompts,
  parsePlotStructures,
  eventLinesPrompts,
  parseEventLines,
  outlinesPrompts,
...
export function parseWithFallback<T>(
  ...
  // JSON fallback
  try {
    return { data: JSON.parse(text), failed: false };
  } catch {}
...
export interface ScenarioConfig {
  ...
  parse: (
    text: string,
    context?: Record<string, unknown>
  ) => { data: unknown; failed: boolean; raw?: string };
  maxTokens: number;
}
...
SCENARIOS['AI生成世界观'] = {
  buildSystemPrompt: ctx => {
    ...
    if (c?._mode === 'section' && c.fieldName) {
      return worldviewSectionPrompt(
        c.fieldName as string,
        (c.existingData as Record<string, string>) || {}
      ).system;
    }
    return worldviewPrompts.buildSystemPrompt();
  },
  buildUserMessage: (input, ctx) => {
    ...
    return worldviewPrompts.buildUserMessage(input);
  },
  parse: (text, ctx?) => {
    const c = ctx as Record<string, unknown> | undefined;
    if (c?._mode === 'section' && c.fieldName) {
      return parseWorldviewSection(text, c.fieldName as string);
    }
    return parseWorldview(text);
  },
...
SCENARIOS['AI生成故事核心'] = {
  ...
  buildUserMessage: (input, ctx) =>
    storyCorePrompts.buildUser(input, ctx as any),
...
SCENARIOS['AI生成物品'] = {
  buildSystemPrompt: ctx =>
    itemsPrompts.buildSystem(ctx as any, (ctx as any)?.已有物品名称),
